...
import static com.jspark.zerobase_project02.type.AccountStatus.IN_USE;
...
class TransactionServiceTest {
    ...
    @Test
    @DisplayName("잔액보다 큰 거래금액 - 잔액 사용 실패")
    void useBalanceFailed_exeedAmount() {
        //given
        AccountUser user = AccountUser.builder()
                .name("Pobi").build();
        user.setId(12L);
    ...
    void cancelTransaction_TransactionNofFound() {
        //given
        given(transactionRepository.findByTransactionId(anyString()))
                .willReturn(Optional.empty());
    ...
    @Test
    @DisplayName("거래와 계좌 매칭실패 - 잔액 사용 취소 실패")
    ...
    @Test
    @DisplayName("1년이 더 경과한 거래취소 시도 - 잔액 사용 취소 실패")
    ...
    void queryTransaction_TransactionNofFound() {
        //given
        given(transactionRepository.findByTransactionId(anyString()))
                .willReturn(Optional.empty());
